        ...
        <div class="dashboard-nav">
            ...
            <nav class="dashboard-nav-list">
                @if (auth()->user()->roles[0]->name === 'super admin' ||
                        auth()->user()->roles[0]->name === 'personal' ||
                        auth()->user()->roles[0]->name === 'agency')
                    <a href="{{ route('cases') }}"
                        class="dashboard-nav-item {{ Route::currentRouteName() == 'cases' ? 'active' : '' }}">
                ...
                @if (auth()->user()->roles[0]->name === 'back office' || auth()->user()->roles[0]->name === 'support')
                    <a href="{{ route('agencies') }}"
                        class="dashboard-nav-item {{ Route::currentRouteName() == 'agencies' ? 'active' : '' }}">
                        <img class="black-icon10" alt="" src="{{ asset('images/dashboard/black10.svg')}}">
                        Agencies
                    </a>
                @endif
                <a href="{{ route('profile') }}"
                    class="dashboard-nav-item {{ Route::currentRouteName() == 'profile' ? 'active' : '' }}">
                ...
                @endif
                @if (auth()->user()->roles[0]->name === 'back office' ||
                        auth()->user()->roles[0]->name === 'support' ||
                        auth()->user()->roles[0]->name === 'agency' ||
                        auth()->user()->roles[0]->name === 'personal')

                    <a href="{{ route('share') }}"
                        class="dashboard-nav-item {{ Route::currentRouteName() == 'share' ? 'active' : '' }}">
                ...
                <a href="{{ route('chat') }}"
                    class="dashboard-nav-item {{ Route::currentRouteName() == 'chat' ? 'active' : '' }}">
                    <img class="black-icon10" alt="" src="{{ asset('images/dashboard/black2.svg')}}">
                    Message  
                </a>
                @if (auth()->user()->roles[0]->name === 'agency' || auth()->user()->roles[0]->name === 'personal')
                    <a href={{ route('share_agency') }}
                        class="dashboard-nav-item {{ Route::currentRouteName() == 'share_agency' ? 'active' : '' }}">
                ...
                @if (auth()->user()->roles[0]->name === 'super admin')
                    <a href={{ route('users') }}
                        class="dashboard-nav-item {{ Route::currentRouteName() == 'users' ? 'active' : '' }}">
                        <img class="black-icon10" alt="" src="{{ asset('images/dashboard/black9.svg')}}">
                        All Users
                    </a>
                @endif
                ...
                <div class="col-4 text-end">
                    @if (auth()->user()->profile)
                        <img class="dashboard-agency-account-child"
                            ...
                            src="{{ asset('upload/profile/' . auth()->user()->profile) }}" />
                    @else
                        <img class="dashboard-agency-account-child"
        ...
        function changeImage(uploadImage, previewImage) {
            document.getElementById(uploadImage).addEventListener('change', function(event) {
                ...
                if (input.files && input.files[0]) {
                    ...
                    alert1.innerHTML =
                    '<p>Please click save for image update</p>'; // Use innerHTML to set HTML content
