...
@section('content')
    ...
    <div class='row justify-content-between mb-4'>
        ...
        @if (!empty($userId))
            ...
            @endphp
            @if ($userShare->role == 'personal' && $countCases > 0)
                    ...
                    @foreach ($cases as $case)
                        <tr>
                            ...
                            <td class="text-end d-flex align-items-center justify-content-around">
                                <a href="{{ route('fed-case.print', $case->id) }}"><button><img src="{{ asset('images/dashboard/black3.svg')}}" /> Print</button></a>
                                <a href="{{ route('fed-case.show', $case->id) }}"><button><img
                                            src="{{ asset('images/dashboard/vector.svg')}} "  />Present</button></a>
                                            <a href="{{route('fed-case.downloadPDF',$case->id)}}"><button><img
                                                src="{{ asset('images/dashboard/pdf.svg')}} "  />Download PDF</button></a>
                                ...
                                @else
                                    <div class="col-4 text-end">
                                        <a href="{{ route('fed-case.edit', $case->id) }}"><button><i
                                                    class='fa fa-edit'></i>
                                                Edit</button></a>
                                    </div>
                                @endif
                                ...
                                @if (!empty($userId))
                                    <button type="button" data-bs-toggle="modal"
                                        ...
                                        @if ($userShare->role == 'personal' && $countCases > 0) style="display: none;" @endif><i
    ...
    <script>
        $(document).ready(function() {
            var table = $('#table-case').DataTable({
                ...
                paging: false,
                language: {
                    emptyTable: "You haven't created any case yet..."
                },
            });
